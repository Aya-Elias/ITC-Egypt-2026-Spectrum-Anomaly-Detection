...
class AgentConfig:
    """Top-level runtime configuration for the SADAR agent.

    Paths may be provided as strings or ``Path`` objects. Values are normalized
    and validated in ``__post_init__`` so API, tests, and scripts share one
    reliable configuration contract.
    """
    ...
    ollama_timeout: int = field(default_factory=lambda: env_int("OLLAMA_TIMEOUT", 120))
    use_hf_embeddings: bool = field(default_factory=lambda: env_bool("SADAR_USE_HF_EMBEDDINGS", False))
    ...
    def ensure_dirs(self) -> None:
        """Create runtime directories needed by the agent."""
        self.cache_dir.mkdir(parents=True, exist_ok=True)

    def to_dict(self) -> dict[str, Any]:
        """Return a JSON-friendly representation without secrets."""
        return {
            "name": self.name,
