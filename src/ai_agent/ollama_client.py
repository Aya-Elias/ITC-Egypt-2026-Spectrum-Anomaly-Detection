...
class OllamaConfig:
    ...
    def from_agent_config(cls, config: AgentConfig) -> "OllamaConfig":
        return cls(base_url=config.ollama_base_url, model=config.ollama_model, timeout=config.ollama_timeout)


class OllamaClient:
    """Minimal dependency-free Ollama API client."""

    def __init__(self, config: OllamaConfig | None = None, agent_config: AgentConfig | None = None) -> None:
    ...
    def _request(self, method: str, path: str, payload: dict[str, Any] | None) -> dict[str, Any]:
        ...
        try:
            ...
            raise LLMUnavailableError(
                f"Ollama is not reachable at {self.base_url}. Start it and run: ollama pull {self.config.model}"
            ) from exc
